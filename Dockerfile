...
RUN echo "Listen 8080" > /etc/apache2/ports.conf \
    && sed -i 's/Listen 80/Listen 8080/g' /etc/apache2/sites-available/000-default.conf \
    && echo "ServerName localhost" >> /etc/apache2/apache2.conf \
    # PHP 설정 완화 - 테스트 목적
    && sed -i 's/display_errors = Off/display_errors = On/g' /etc/php/*/apache2/php.ini 2>/dev/null || true
